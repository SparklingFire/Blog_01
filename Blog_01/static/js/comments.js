...
function comment_builder(response){

    let message = $('<div class="comment-message" id=' + response.comment_pk +'></div>');
    ...
    let message_footer = $('<div class="comment-message-footer"><span class="comment-answer">ответить</span>&#160&#160' +
        ...
        '<span class="glyphicon glyphicon-thumbs-up like" data-grade="like"><a href="/rate/' + response.comment_pk + '/like/"></a></span> ' +
        '<span class="total-likes">' + response.likes + '</span>' + ' ' +
        '<span class="glyphicon glyphicon-thumbs-down like" data-grade="dislike"><a href="/rate/' + response.comment_pk + '/dislike/"></a></span> ' +
    ...
    if (response.login_user == response.username || response.current_session == response.session){
        message_footer.prepend($('<span class="comment-edit" id="' + response.comment_pk + '">редактировать</span>'))
    }
    ...
    comments_list.push(response.comment_pk);
    ...
    $.ajax({
        ...
        success: function (response) {
            response.forEach(function (response) {
                if (comments_list.indexOf(response) == -1){
                    comment_builder(response);
                }
                let children = response.comment_children;
                if (children != undefined){
                children.forEach(function (child) {
                    if (comments_list.indexOf(child) == -1){
                    comment_builder(child);
                    }
                })
                }
            });
        ...
        complete: setTimeout(function() {loadComments(comments_list)}, 10000),
        error: function () {
        }
    })
}
                ...
                success: function (response) {
                    if ('errors' in response){
                        ...
                        if (pk == undefined){
                            ...
                            scrollToComment(comment_builder(response));
                        }
                        else{
                            form.parent().parent().find('.comment-message-text:first').text(form.find('textarea').val());
                            form.hide();
                        }
                        $(this).closest('.comment-edit-hide').attr('class', '.comment-edit');
                        $(this).closest('.comment-answer-hide').attr('class', 'comment-answer');
                        commentsCountRefresher();
                    }
                },
